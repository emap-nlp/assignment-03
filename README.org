...
* Tarefa
  ...
  2. Usar o sistema para fazer a análise sintática dos primeiros
  ...
  Note que esse processo talvez envolva se deparar com algumas sentenças que são anotadas pelo modelo de 
  forma errada. A ideia não é seu código de extração de informação se adequar a dados ruidosos; nesses 
  casos a ideia é corrigir essas sentenças (ou, por exemplo, deixar de lado quando o arquivo não tem o nome dos pais; depende do caso).
...
* Correção de análises sintáticas
  ...
  É recomendável que você trabalhe de modo incremental. Pegue um exemplo de árvore
  e crie um código que consiga extrair as informações desta árvore. Partindo disso, rode
  em outras sentenças, veja os erros, generalize e faça isso até ter um código com um desempenho aceitável.

  É importante notar que, eventualmente, você irá se deparar com análises sintáticas erradas;
  a ideia nesses casos não é fazer com que seu código consiga extrair informações até de análises 
  erradas mas sim conseguir achá-las e corrigi-las. Nesses casos, você pode usar os fórums para 
  tirar dúvidas de análises aparentemente erradas. Se for o caso, você pode retreinar o seu parser
  de dependências com os novos casos manualmente corrigidos junto ao Bosque e conseguir novos 
  parsings (se tudo der certo, estes terão menos erros). Caso isso ocorrá, explicite e documente.
